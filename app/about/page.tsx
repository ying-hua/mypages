    ...
    <div className="px-6 md:px-12 lg:px-24 py-20 max-w-3xl mx-auto">
      ...
      <FadeIn delay={0.1}>
        <h1
          ...
          className="text-5xl md:text-6xl font-black text-[#3A2E26] mb-12 relative inline-block"
        >
          Hi, I&apos;m Astro
      ...
      <FadeIn delay={0.2}>
        <div
          ...
          className="text-[#3A2E26] text-lg leading-[1.9] mb-16 space-y-4"
        >
          <p>
            我是 <strong className="text-[#B5532A]">Astro</strong>，北京大学研三在读，即将毕业。
          </p>
          <p>
          ...
          <p>
            即将以 <strong className="text-[#B5532A]">AI 应用软件工程师</strong> 的身份踏入职场。
      ...
      <FadeIn delay={0.3}>
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {skills.map((group) => (
              <div key={group.category} className="bg-[#F2EBE0] rounded-2xl p-5 border border-[#E8D5C4]">
                <p
                  style={{ fontFamily: "var(--font-fraunces)" }}
                  className="text-sm text-[#B5532A] font-semibold mb-3 tracking-wide uppercase"
                >
                  {group.category}
                </p>
                <ul style={{ fontFamily: "var(--font-lora)" }} className="space-y-1.5">
                  {group.items.map((item) => (
                    <li key={item} className="text-sm text-[#6B5344] flex items-center gap-2">
      ...
      <FadeIn delay={0.4}>
        <div>
          ...
          <div className="relative pl-6">
            {/* Vertical line */}
            <div className="absolute left-0 top-2 bottom-2 w-px bg-[#E8D5C4]" />

            <div className="space-y-8">
              {timeline.map((item, i) => (
                <div key={i} className="relative">
                  {/* Dot */}
                  <div className="absolute -left-[25px] top-1 w-3 h-3 rounded-full bg-[#C68B59] border-2 border-[#FAF6EF]" />
                  ...
                  <p
                    style={{ fontFamily: "var(--font-lora)" }}
                    className="text-[#3A2E26] text-base"
                  >
                    {item.event}
                  </p>
                </div>
      ...
      {/* Contact */}
      <FadeIn delay={0.5}>
        <div className="mt-20 p-8 bg-[#F2EBE0] rounded-2xl border border-[#E8D5C4] text-center">
          <p style={{ fontFamily: "var(--font-caveat)" }} className="text-[#C68B59] text-2xl mb-3">
            来聊聊？
          </p>
          <p style={{ fontFamily: "var(--font-lora)" }} className="text-[#6B5344] mb-5 text-sm">
            无论是合作、交流还是随便聊聊，我的邮箱随时敞开。
          </p>
          <a
            href="mailto:139152599yh@gmail.com"
            style={{ fontFamily: "var(--font-lora)" }}
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-[#B5532A] text-[#FAF6EF] text-sm hover:bg-[#7A4F35] transition-colors"
          >
            发封邮件
          </a>
        </div>
